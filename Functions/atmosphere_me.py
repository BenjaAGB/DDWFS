...
def getATM(wfs,p,atm_path, **kwargs):
    ...
    # beta distribution
    r = p['r0'][0]+(p['r0'][1]-p['r0'][0])*stats.beta.rvs(p['ab'][0],p['ab'][1], size=nData, random_state=random_state)
    fig, ax = plt.subplots()
    ax.hist(r, bins=30, density=True, alpha=0.6, color='b', edgecolor='k', label='Histogram')
    x = np.linspace(p['r0'][0], p['r0'][1], nData)
    pdf_values = stats.beta.pdf((x - p['r0'][0]) / (p['r0'][1] - p['r0'][0]), p['ab'][0], p['ab'][1]) / (p['r0'][1] - p['r0'][0])
...
def ATM(wfs,r0v=1,nData=1):
    ...
    atm.n_lvl         = 0.1
    atm.binning       = 1
    atm.nTimes        = 2/atm.resAO  #wfs.fovPx/atm.resAO
