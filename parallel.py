...
parser.add_argument('--device',         default = '3,4',    type = str,   help = 'Device to use: cpu or cuda: 0, 1, ..., 7')
...
def ddp_worker(local_rank, gpu_ids, port, args):
...
if __name__ == "__main__":
    ...
    if backend == 'cpu':
        ...
        params.device = str(gpu_ids[0])
        run(params)
    else:
        port = pick_port()
        mp.spawn(ddp_worker, nprocs = len(gpu_ids), args = (gpu_ids, port, params))